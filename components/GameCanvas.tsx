  ...
  if (gameState === GameState.START_MENU) {
    return (
      <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
        ...
        <div className="ea-fade-in w-[92%] max-w-xl rounded-2xl border border-[#c8a24a]/50 bg-[#0b0f1a]/90 p-6 text-center shadow-2xl">
          <p className="text-sm uppercase tracking-[0.3em] text-[#c0c7d1]">First Degree</p>
          <h1 className="ea-display ea-glow mt-2 text-3xl md:text-4xl text-[#c8a24a]">Entered Apprentice</h1>
          <p className="mt-4 text-base text-[#c0c7d1]">
            You stand at the door of Freemasonry. Collect the working tools, answer the questions, and prove your proficiency.
          </p>
    ...
    <div className="relative w-full h-full overflow-hidden no-select">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-10 pointer-events-none">
        ...
        <div className="bg-slate-800/80 px-3 py-2 rounded-lg border border-slate-600 backdrop-blur-sm flex items-center gap-3">
          {/* Tassels Tracker */}
          <div className="flex gap-1 mr-2 border-r border-slate-600 pr-3">
               {[101,102,103,104].map(id => (
                   <div key={id} className={`w-2.5 h-2.5 rounded-full ${collectedTassels.has(id) ? 'bg-amber-400 shadow-[0_0_6px_#fbbf24]' : 'bg-slate-700'}`} title="Cardinal Virtue"></div>
               ))}
          </div>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/95 backdrop-blur-md p-4 animate-in fade-in duration-500">
          <div className="relative w-full max-w-xl max-h-[95vh] flex flex-col items-center bg-slate-900 border-2 md:border-4 border-amber-500 rounded-xl p-4 md:p-8 shadow-2xl text-center overflow-y-auto">
            ...
            <p className="text-slate-400 text-sm md:text-base mb-2 italic">
              You have proven your proficiency in the First Degree.
            </p>

            <p className="text-amber-500 text-base md:text-lg mb-6 font-semibold">
              The door to the Fellow Craft Degree is now open to you.
            </p>
            ...
            <div className="w-full bg-slate-800/80 p-4 md:p-6 rounded-lg border border-slate-700 mb-4">
              ...
              {collectedTassels.size === 4 && (
                <div className="bg-amber-900/30 p-3 rounded border border-amber-600 mb-4">
                  <p className="text-amber-400 font-bold">Perfect Ashlar Bonus: +1000</p>
                  <p className="text-slate-400 text-sm">All four Cardinal Virtues collected!</p>
                </div>
              )}
                ...
                {[101,102,103,104].map(id => (
                  <div
                    ...
                    className={`w-6 h-6 md:w-8 md:h-8 rounded-full border-2 ${
                      collectedTassels.has(id)
                        ? 'bg-amber-400 border-amber-300 shadow-[0_0_12px_#fbbf24]'
                        : 'bg-slate-700 border-slate-600'
                    }`}
                    title={collectedTassels.has(id) ? 'Collected' : 'Not Collected'}
                  />
                ))}
              </div>
              <p className="text-slate-500 text-xs mt-2">Cardinal Virtues</p>
            </div>
